...
def _parse_pfp_target(text: str) -> Optional[str]:
    """Extract a username or numeric ID from a /pfp argument."""
    text = text.strip()
...
async def cmd_pfp(_: Client, message: Message) -> None:
    ...
    if len(args) < 2:
        reply = await message.reply(
            "⚠️ <b>Usage:</b> <code>/pfp @username</code>\n"
            "Works for any public channel, group, or user.\n\n"
            "<i>Example:</i> <code>/pfp @durov</code>"
        )
    ...
    status = await message.reply(f"🔍 <b>Fetching profile photos for @{target}…</b>")
    tmp_paths: list[str] = []
    ...
    try:
        ...
        photos = []
        try:
            async for photo in user.get_chat_photos(target, limit=20):
                photos.append(photo)
        except Exception as e:
            err = str(e).lower()
            if any(k in err for k in ("username", "peer", "not found",
                                      "invalid", "private")):
                await status.edit_text(
                    f"❌ <b>Could not find:</b> <code>{target}</code>\n"
        ...
        if not photos:
            await status.edit_text(
                f"ℹ️ <b>@{target}</b> has no profile photos."
            )
            schedule_delete(status)
            return
        ...
        total = len(photos)
        await status.edit_text(
            f"⬇️ <b>Downloading {total} profile photo(s)…</b>"
        )
        ...
        await status.edit_text(
            f"⬆️ <b>Sending {len(tmp_paths)} photo(s)…</b>"
        )
        ...
        if len(tmp_paths) == 1:
            await bot.send_photo(
                uid,
                photo=tmp_paths[0],
                caption=f"📷 <b>@{target}</b> — profile photo",
            )
        else:
            for batch_start in range(0, len(tmp_paths), 10):
                ...
                for j, p in enumerate(batch):
                    cap = (
                        f"📷 <b>@{target}</b> — {batch_start + j + 1}/{len(tmp_paths)}"
                        if j == 0 else ""
                    )
        ...
        await log_to_channel(
            ...
            f"User: {message.from_user.mention} (<code>{uid}</code>)\n"
            f"Target: @{target} | Photos: {len(tmp_paths)}",
        )
        try:
            await status.delete()
        except Exception:
        ...
        logger.error("[PFP] Unexpected error for %s: %s", target, e)
        try:
            await status.edit_text(
