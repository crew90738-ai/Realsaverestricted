...
# ── aiohttp for thumbnail fetching (already installed for health server) ───────
try:
...
class Config:
    ...
    # Limits / tunables
    MAX_FILE_SIZE: int             = 4 * 1024 * 1024 * 1024   # 4 GB
...
async def redis_total_users() -> int:
    return await _get_redis().scard("bot_users")
...
user = Client(
    ...
    parse_mode=ParseMode.HTML,
    workers=4,
    no_updates=True,   # user client only fetches messages on demand
)


# ═══════════════════════════════════════════════════════════════════════════════
#  CORE: PROCESS A SINGLE RESTRICTED MESSAGE
#  (used by both the link handler and the batch handler)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def process_single_message(
    ...
    """
    Fetch one message via the user client, download its media,
    and re-upload it via the bot to dest_chat_id.
...
# ── Inline callbacks for /start keyboard ─────────────────────────────────────
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HANDLERS — THUMBNAIL  (/setthumb  /delthumb  /showthumb)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HANDLERS — ADMIN  (/stats  /broadcast  /addpremium  /removepremium)
# ═══════════════════════════════════════════════════════════════════════════════
...
async def cmd_addpremium(_: Client, message: Message) -> None:
    ...
    try:
        await bot.send_message(
            uid, "🌟 <b>You've been granted Premium access!</b>"
        )
    except Exception:
        pass
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HANDLER — Telegram link forwarder  (group=0, highest priority for text msgs)
# ═══════════════════════════════════════════════════════════════════════════════
...
# ═══════════════════════════════════════════════════════════════════════════════
#  HANDLER — Batch state-machine interceptor  (group=2, lowest priority)
#
#  Runs AFTER the Telegram link handler (group=0) and yt-dlp (group=1).
#  If the user is in a batch flow, this handler "claims" the message and
#  raises StopPropagation so nothing else fires.
# ═══════════════════════════════════════════════════════════════════════════════
...
async def batch_state_handler(_: Client, message: Message) -> None:
    ...
    if not state:
        return   # not in a batch flow

    step = state.get("step")

    # ── Waiting for START link ────────────────────────────────────────────────
    ...
    # ── Waiting for END link ──────────────────────────────────────────────────
    elif step == "awaiting_end":
        if not is_telegram_link(text):
            reply = await message.reply("❌ Send a valid Telegram message link (the end of the range).")
            schedule_delete(reply)
            raise StopPropagation
        ...
        if total > Config.BATCH_MAX:
            reply = await message.reply(
                ...
                f"Max allowed: {Config.BATCH_MAX}. Try a smaller range."
            )
            schedule_delete(reply)
            await redis_clear_batch(uid)
            raise StopPropagation
...
async def main() -> None:
    ...
    # 1. Health server (bind port before Render's first health check)
    ...
    # 4. Keep running until Ctrl-C or SIGTERM
    # pyrogram.idle() is the officially recommended way to keep
    # a Pyrogram bot alive — it handles OS signals correctly
