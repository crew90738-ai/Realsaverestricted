...
def start_health_server() -> None:
    ...
    logger.info("Health server on port %d ✓", Config.PORT)
...
async def process_single_message(
    ...
    try:
        ...
        if status_msg: await status_msg.edit_text("❌ User account is not in this private channel.")
        return None
    except (ChannelInvalid, UsernameInvalid, UsernameNotOccupied):
        if status_msg: await status_msg.edit_text("❌ Invalid channel — check the link.")
        return None
    except MessageIdInvalid:
        if status_msg: await status_msg.edit_text(f"❌ Message #{msg_id} not found.")
        return None
    ...
    if not media_type:
        # Text-only message — only send if there's actual content
        if not caption or not caption.strip():
            return None   # skip empty/no-content messages silently
        sent = await bot.send_message(dest_chat_id, caption)
        await redis_inc_downloads()
        return sent

    if status_msg: await status_msg.edit_text("📥 <b>Downloading…</b>")
    dl_start = time.time()

    try:
        dl_cb = make_progress_callback(status_msg, "Downloading", dl_start) if status_msg else None
        file_path = await user.download_media(src, file_name=Config.DOWNLOAD_DIR + "/", progress=dl_cb)
    except Exception as e:
        if status_msg: await status_msg.edit_text(f"❌ Download failed: <code>{e}</code>")
        return None

    if not file_path or not os.path.exists(file_path):
        if status_msg: await status_msg.edit_text("❌ File not found after download.")
        return None

    file_size = os.path.getsize(file_path)
    if file_size > Config.MAX_FILE_SIZE:
        os.remove(file_path)
        if status_msg: await status_msg.edit_text(f"❌ File too large: {humanbytes(file_size)}.")
        return None
    ...
    if thumb_fid:
        try:
            thumb_path = await bot.download_media(thumb_fid, file_name=Config.DOWNLOAD_DIR + "/thumb_")
        except Exception:
            thumb_path = None
    ...
    try:
        if media_type == "video":
            ...
            vn = src.voice
            sent = await bot.send_voice(voice=file_path, duration=vn.duration if vn else 0, **kw)
        ...
        if status_msg: await status_msg.edit_text(f"❌ Upload failed: <code>{e}</code>")
    finally:
...
async def cb_check_sub(_: Client, query: CallbackQuery) -> None:
    if await check_force_sub(bot, query.from_user.id):
        await query.message.edit_text("✅ <b>Verified!</b> Send me a link or video URL.")
    else:
...
async def cmd_setthumb(_: Client, message: Message) -> None:
    ...
    if not target.photo:
        reply = await message.reply("📸 Reply to a <b>photo</b> with <code>/setthumb</code>.")
        schedule_delete(reply)
        return
...
async def cmd_showthumb(_: Client, message: Message) -> None:
    ...
    if not fid:
        reply = await message.reply("ℹ️ No thumbnail set. Use /setthumb (reply to a photo).")
        schedule_delete(reply)
        return
    try:
        sent = await bot.send_photo(message.from_user.id, fid, caption="🖼️ <b>Your current thumbnail</b>")
...
async def main() -> None:
    ...
    # Warm up the peer cache by loading all dialogs the user account has joined.
    # This is REQUIRED because Pyrogram can't resolve private channel IDs without
    # their access_hash, which only comes from the dialogs list.
    # Official Pyrogram docs: https://docs.pyrogram.org/faq/peer-id-invalid-error
    logger.info("Loading user dialogs to populate peer cache (may take a moment)…")
    try:
        count = 0
    ...
    # 3. Keep alive using Pyrogram's own idle() — handles SIGINT/SIGTERM correctly
    await idle()

    # 4. Graceful shutdown (idle() returned because of a stop signal)
