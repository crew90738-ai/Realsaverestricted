...
# ─── Group tracking (for global repeat / keep-alive) ─────────────────────────
...
async def gr_start_worker() -> None:
    ...
    global _gr_task, _gr_queue_task
    ...
    # Start heartbeat
    hb = asyncio.create_task(_gr_heartbeat_worker())
    hb.add_done_callback(lambda t: t.exception() if not t.cancelled() else None)
...
async def gr_stop_worker() -> None:
    """Stop main repeat worker. Queue worker and heartbeat stay alive."""
    global _gr_task
...
async def handle_new_chat_members(_: Client, message: Message) -> None:
    ...
    adder_info = (
        f"{adder.full_name} (<code>{adder.id}</code>)"
...
async def _send_gr_menu(chat_id: int, edit_msg_id: Optional[int] = None) -> None:
    """Build and send (or edit) the global repeat menu."""
    ...
    menu_text = (
        ...
        f"<b>Status:</b> {'✅ Running' if (enabled and worker_run) else ('⏸ Enabled (starting…)' if enabled else '❌ Stopped')}\n"
        f"<b>Groups:</b> {grp_count}\n"
        f"<b>Interval:</b> {interval_str}\n"
        f"<b>Message:</b> {text_preview}\n"
        f"<b>Auto-delete prev:</b> {'✅ ON' if autodel else '❌ OFF'}\n"
        f"<b>Self-delete after:</b> {f'{selfdel}s' if selfdel else '❌ OFF'}\n\n"
        f"<i>ℹ️ Add the bot to Telegram groups — it will post the repeat message "
        f"to each group at the set interval, keeping Render alive automatically.</i>"
    )
    ...
    kb = InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("💣 Set Self-Delete", callback_data="gr_set_selfdelete"),
    ...
    await bot.send_message(chat_id, menu_text, reply_markup=kb)
...
async def main() -> None:
    ...
    def _handle_exception(loop, context):
        ...
        if exc:
            # Suppress harmless "Task destroyed while pending" warnings from
            # _auto_delete tasks that are cancelled on shutdown
            if isinstance(exc, asyncio.CancelledError):
                return
            if "auto_delete" in str(context.get("future", "")):
                return
