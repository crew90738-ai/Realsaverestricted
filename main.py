...
async def handle_story_download(message: Message, url: str) -> None:
    ...
    try:
        ...
        try:
            ...
            # Variant A: GetPeerStories(peer=) — Layer 166+
            try:
                from pyrogram.raw.functions.stories import GetPeerStories
                r = await user.invoke(GetPeerStories(peer=peer))
                stories_list = r.stories
            except Exception as e:
                last_err = e

            # Variant B: GetUserStories(user_id=) — Layer 160-165
            if stories_list is None:
                try:
                    ...
                    # r is UserStories; r.stories may be the list OR another
                    # UserStories wrapper depending on build — unwrap safely
                    raw = r.stories
                    stories_list = raw.stories if hasattr(raw, 'stories') else list(raw)
                except Exception as e:
                    last_err = e

            # Variant C: GetStoriesByID(user_id=, id=) — some intermediate layer
            if stories_list is None:
                try:
            ...
            # Variant D: GetStoriesByID(peer=, id=) — another layer variant
            if stories_list is None:
                try:
                    from pyrogram.raw.functions.stories import GetStoriesByID
            ...
            if stories_list is None:
                # Show exactly what IS available so this can be fixed definitively
                import pyrogram.raw.functions.stories as _sf
                avail = [x for x in dir(_sf) if x[0].isupper()]
                raise Exception(
                    f"No compatible story fetch API found in this build.\n"
            ...
            if not isinstance(story, StoryItem) or not story.media:
                await status.edit_text(
                    "❌ <b>Story not found or expired.</b>\n"
                    "Telegram stories expire after 24 hours."
                )
        ...
        tmp_path = os.path.join(Config.DOWNLOAD_DIR, f"story_{uid}_{story_id}.{ext}")
        dl_cb    = make_progress_callback(status, "⬇️ Downloading story")
        tmp_path = await user.download_media(raw_media, file_name=tmp_path, progress=dl_cb)
