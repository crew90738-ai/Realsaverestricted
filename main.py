            ...
            try:
                ...
                if not media_type:
                    ...
                    if raw_text is not None and str(raw_text).strip():
                        ...
                        # Try to get HTML-formatted text; fall back to plain if .html fails
                        try:
                            send_text  = raw_text.html
                            _parse     = ParseMode.HTML
                        except Exception:
                            send_text  = str(raw_text)
                            _parse     = None

                        # Chunk into ≤4096-char pieces
                        chunks = [send_text[j:j + 4096] for j in range(0, len(send_text), 4096)]
                        _ok = True
                        for chunk in chunks:
                            try:
                                await bot.send_message(
                                ...
                                logger.warning(
                                    "[DLBOT] HTML send failed uid %d, retrying as plain: %s", uid, e
                                )
                                # HTML was rejected — retry the same chunk as plain text
                                plain_chunk = str(raw_text)[
                                    chunks.index(chunk) * 4096:
                                    (chunks.index(chunk) + 1) * 4096
                                ]
                                try:
                                    await bot.send_message(
                                        uid, plain_chunk,
                                    ...
                                    logger.error(
                                        "[DLBOT] Plain text fallback also failed uid %d: %s", uid, e2
                                    )
                                    _ok = False
                                    break
                        if _ok:
