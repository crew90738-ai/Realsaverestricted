...
async def _process_inner(
    ...
    # ── 5. Text-only message ─────────────────────────────────────────────────
    if not media_type:
        full_text = (custom_cap or src.text or "").strip()
        if not full_text:
            if status_msg:
                await status_msg.edit_text(
            ...
            return None
        # Split into 4096-char chunks (Telegram's send_message limit).
        # Use ParseMode.DISABLED so raw text with <, >, & characters is never
        # misinterpreted as HTML, which would silently truncate the message.
        chunks = [full_text[i:i + 4096] for i in range(0, len(full_text), 4096)]
        sent = None
        for chunk in chunks:
            try:
                sent = await bot.send_message(
                    dest_chat_id, chunk, parse_mode=ParseMode.DISABLED
                )
            except Exception as e:
                logger.error("[TEXT] Chunk send failed: %s", e)
