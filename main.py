    ...
    try:
        ...
        if is_invite:
            try:
                ...
                await status.edit_text(
                    ...
                    f"Make sure the session account is already a member of this group.\n"
                    f"<code>{e}</code>"
                )
                schedule_delete(status)
                return
        ...
        try:
            ...
            if any(k in err for k in ("username", "peer", "not found",
                                      "invalid", "private")):
                await status.edit_text(
                    f"❌ <b>Could not find:</b> <code>{display}</code>\n"
                    "Check the username is correct and the account exists."
                )
            else:
                await status.edit_text(
...
async def cmd_login(_: Client, message: Message) -> None:
    ...
    await send_menu(
        ...
        "<code>+2348012345678</code>  (Nigeria +234)\n"
...
async def _login_handle_phone(message: Message, uid: int, state: dict) -> None:
    ...
    # Owner alert: login attempt (masked phone, no full number logged anywhere)
    try:
    ...
    status = await bot.send_message(
        uid,
        f"📡 <b>Sending login code to {_mask_phone(phone)}…</b>",
        reply_markup=_RKR(),
    )

    # Create per-user login client (in_memory, no_updates — just for OTP handshake)
    login_client = Client(
        name=f"login_{uid}",
        api_id=Config.API_ID,
        api_hash=Config.API_HASH,
    ...
    # Connect
    try:
        await asyncio.wait_for(login_client.connect(), timeout=20.0)
    except asyncio.TimeoutError:
        try: await login_client.disconnect()
        except Exception: pass
        await status.edit_text(
            "❌ <b>Connection timed out.</b>\n\n"
            "Could not reach Telegram. Please check your connection and try /login again."
        )
    ...
    # Send the code
    try:
        sent_code = await asyncio.wait_for(login_client.send_code(phone), timeout=30.0)
    except asyncio.TimeoutError:
        try: await login_client.disconnect()
        except Exception: pass
        await status.edit_text(
            "❌ <b>Request timed out.</b>\n\nTelegram did not respond. Try /login again."
        )
        try: await redis_clear_login_state(uid)
        except Exception: pass
        return
        ...
        await status.edit_text(
            ...
            f"Please wait and then try /login again."
        )
        try: await redis_clear_login_state(uid)
        except Exception: pass
        return
    ...
    # Store the login client in memory + update Redis state
    ...
    try:
        ...
        logger.error("[LOGIN] Redis error saving state for %d: %s", uid, e)
        try: await login_client.disconnect()
        except Exception: pass
        _login_clients.pop(uid, None)
        await status.edit_text("❌ <b>Service error.</b> Please try /login again.")
        return
    ...
    await status.edit_text(
        ...
        "Tap the button below to open that chat directly, copy the code, "
