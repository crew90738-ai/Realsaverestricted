...
async def handle_story_download(message: Message, url: str) -> None:
    ...
    try:
        ...
        # Strategy 1: FileId encoding → download_media(string)
        try:
        ...
        # Strategy 2: _download_file with InputFileLocation
        if not tmp_path or not os.path.exists(tmp_path):
            try:
                ...
                if media_type == "photo":
                    ...
                    sz, dc = getattr(best, 'size', 0), raw_media.dc_id
                else:
                    loc  = InputDocumentFileLocation(
                        id=raw_media.id, access_hash=raw_media.access_hash,
                        file_reference=raw_media.file_reference, thumb_size="",
                    )
                    sz, dc = getattr(raw_media, 'size', 0), raw_media.dc_id
                for dl_args in [
                    ...
                    try:
                        ...
                        dl_errors.append(f"S2({type(e2).__name__}): {e2}")
            except Exception as e:
                dl_errors.append(f"S2-setup({type(e).__name__}): {e}")
