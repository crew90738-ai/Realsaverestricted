    ...
    try:
        ...
        for i, src_msg in enumerate(collected_msgs, 1):
            ...
            try:
                ...
                # ── Text-only message ─────────────────────────────────────────
                if not media_type:
                    # msg.text is None for some bot messages — also check caption
                    raw_text = src_msg.text if src_msg.text is not None else src_msg.caption
                    logger.info(
                        "[DLBOT] msg %d/%d id=%d has_text=%s has_caption=%s",
                        i, total, src_msg.id,
                        src_msg.text is not None,
                        src_msg.caption is not None,
                    )
                    if not raw_text or not str(raw_text).strip():
                        logger.info("[DLBOT] msg %d/%d skipped (no text content)", i, total)
                        if i < total:
                    ...
                    try:
                        await status.edit_text(
                            f"📨 <b>Sending text {i}/{total}…</b>"
                        )
                    except Exception:
                        pass

                    # Try HTML first; fall back to plain if .html fails or Telegram rejects it
                    try:
                        html_text = raw_text.html
                    except Exception as e:
                        logger.warning("[DLBOT] .html failed for msg %d: %s", i, e)
                        html_text = None
                    ...
                    if html_text and html_text.strip():
                        chunks = [html_text[j:j + 4096] for j in range(0, len(html_text), 4096)]
                        for chunk in chunks:
                            try:
                                await bot.send_message(
                                    uid, chunk,
                                ...
                                logger.warning(
                                    "[DLBOT] HTML rejected for msg %d, trying plain: %s", i, e
                                )
                                # HTML rejected — fall through to plain send below
                                html_text = None
                                break

                    if not html_text or not html_text.strip():
                        # Plain text fallback
                        chunks = [plain_text[j:j + 4096] for j in range(0, len(plain_text), 4096)]
                        for chunk in chunks:
                            try:
                                await bot.send_message(
                                    uid, chunk,
                                ...
                                logger.error(
                                    "[DLBOT] Plain text also failed for msg %d: %s", i, e2
                                )
                                _ok = False
                                break
