...
async def handle_story_download(message: Message, url: str) -> None:
    """Fetch a Telegram story via the userbot and deliver it unrestricted."""
    uid    = message.from_user.id
    ...
    try:
        # ── 1. Fetch story via userbot ───────────────────────────────────────
        try:
            stories = await user.get_stories(username, story_ids=[story_id])
        except Exception as e:
            err = str(e).lower()
        ...
        story      = stories[0]
        media      = story.photo or story.video
        if not media:
            await status.edit_text(
                "❌ <b>This story has no downloadable media.</b>\n"
                "(Text-only or unsupported type)"
            )
            schedule_delete(status)
            return

        media_type = "photo" if story.photo else "video"
        ...
        tmp_path = await user.download_media(media, file_name=tmp_path, progress=dl_cb)
