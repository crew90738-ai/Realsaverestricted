...
# ─── Bot response helpers (non-owner gate) ───────────────────────────────────

_DEFAULT_BOT_RESPONSE = "🚫 <b>Bot is currently unavailable.</b>"

async def redis_get_bot_response() -> str:
    try:
        val = await _r().get("bot_response_text")
        return val if val else _DEFAULT_BOT_RESPONSE
    except Exception as e:
        logger.error("[REDIS] get_bot_response: %s", e)
        return _DEFAULT_BOT_RESPONSE

async def redis_set_bot_response(text: str) -> None:
    try:
        await _r().set("bot_response_text", text)
    except Exception as e:
        logger.error("[REDIS] set_bot_response: %s", e)

async def redis_get_bot_response_timer() -> int:
    """Returns seconds. 0 = never delete."""
    try:
        val = await _r().get("bot_response_timer")
        return int(val) if val else 0
    except Exception as e:
        logger.error("[REDIS] get_bot_response_timer: %s", e)
        return 0

async def redis_set_bot_response_timer(seconds: int) -> None:
    try:
        await _r().set("bot_response_timer", str(seconds))
    except Exception as e:
        logger.error("[REDIS] set_bot_response_timer: %s", e)

async def redis_set_owner_state(uid: int, state: dict) -> None:
    try:
        await _r().set(f"owner_state:{uid}", json.dumps(state), ex=600)
    except Exception as e:
        logger.error("[REDIS] set_owner_state(%d): %s", uid, e)

async def redis_get_owner_state(uid: int) -> Optional[dict]:
    try:
        raw = await _r().get(f"owner_state:{uid}")
        return json.loads(raw) if raw else None
    except Exception as e:
        logger.error("[REDIS] get_owner_state(%d): %s", uid, e)
        return None

async def redis_clear_owner_state(uid: int) -> None:
    try:
        await _r().delete(f"owner_state:{uid}")
    except Exception as e:
        logger.error("[REDIS] clear_owner_state(%d): %s", uid, e)
...
_PUBLIC_RE  = re.compile(r"t\.me/([a-zA-Z][a-zA-Z0-9_]{3,})/(?:\d+/)?(\d+)")
...
async def _process_inner(
    ...
    caption    = (custom_cap or (src.caption.html if src.caption else "") or (src.text.html if src.text else "") or "")[:1024]
    ...
    kw    = dict(chat_id=dest_chat_id, caption=caption, parse_mode=ParseMode.HTML, progress=up_cb)
...
def _start_kb(is_owner: bool = False, has_session: bool = False) -> InlineKeyboardMarkup:
    ...
    if is_owner:
        rows.append([InlineKeyboardButton(
            "⚙️ Bot Response", callback_data="bot_response_menu"
        )])
        rows.append([InlineKeyboardButton(
...
# ═══════════════════════════════════════════════════════════════════════════════
#  NON-OWNER GATE (group=-3, highest priority)
#  Every message from any user that is NOT the owner is blocked here.
#  Their message is deleted and they receive the configured custom response.
# ═══════════════════════════════════════════════════════════════════════════════

def _format_timer(seconds: int) -> str:
    """Human-readable timer label."""
    if seconds == 0:   return "Never (manual dismiss)"
...
@bot.on_message(filters.private & ~filters.user(Config.OWNER_ID), group=-3)
async def non_owner_gate(_: Client, message: Message) -> None:
    """Block all non-owner users. Delete their message, send custom response."""
    ...
    # Fetch custom response text and timer
    try:
        text  = await redis_get_bot_response()
        timer = await redis_get_bot_response_timer()
    except Exception:
        text  = _DEFAULT_BOT_RESPONSE
        timer = 0

    # Send the response — protect_content so they can't forward or save it
    try:
        reply = await bot.send_message(
            uid,
            text,
        ...
        if timer > 0:
            schedule_delete(reply, delay=timer)
    except Exception as e:
        logger.error("[GATE] Failed to send response to %d: %s", uid, e)
...
async def cmd_dlbot(_: Client, message: Message) -> None:
    ...
    status = await message.reply(
        ...
        f"Looking for up to {count} message(s)."
    )

    try:
        # ── 1. Collect messages (all types) ──────────────────────────────────
        collected_msgs: list[Message] = []

        try:
            async for msg in user.get_chat_history(target_raw, limit=count):
                collected_msgs.append(msg)
                if len(collected_msgs) >= count:
                    break
        except FloodWait as fw:
            await status.edit_text(
        ...
        if not collected_msgs:
            await status.edit_text(
                f"ℹ️ <b>No messages found</b> in chat with @{target_raw}.\n\n"
                "Possible reasons:\n"
                "• The bot hasn't sent any messages yet\n"
                f"• Try increasing the count: <code>/dlbot @{target_raw} 50</code>"
            )
            schedule_delete(status)
            return

        total = len(collected_msgs)
        await status.edit_text(
            f"✅ <b>Found {total} message(s).</b> Starting…"
        )

        # ── 2. Send each message ──────────────────────────────────────────────
        #
        #  Root cause of has_text=False / media_type=None:
        #  Old pyrofork silently drops message.text and media fields when it
        #  encounters newer Telegram entity types (expandable blockquotes, etc.).
        #  Fix: read src_msg.raw (the raw MTProto object) as fallback for both
        #  text content and media type detection.
        # ─────────────────────────────────────────────────────────────────────
        sent_ok = 0
        failed  = 0

        _protect = (uid != Config.OWNER_ID)   # owner can save/forward freely

        for i, src_msg in enumerate(collected_msgs, 1):
            file_path:  Optional[str] = None
            thumb_path: Optional[str] = None
            try:
                # ── Determine media type (with raw fallback) ──────────────────
                media_type = get_media_type(src_msg)

                if not media_type:
                    raw_obj  = getattr(src_msg, "raw", None)
                    raw_media = getattr(raw_obj, "media", None) if raw_obj else None
                    if raw_media:
                        raw_cls = type(raw_media).__name__
                        logger.info("[DLBOT] msg %d/%d raw media cls: %s", i, total, raw_cls)
                        if "Photo" in raw_cls:
                            media_type = "photo"
                        elif "Document" in raw_cls:
                            doc = getattr(raw_media, "document", None)
                            attrs = getattr(doc, "attributes", []) if doc else []
                            attr_names = [type(a).__name__ for a in attrs]
                            if any("Video" in n for n in attr_names):
                                media_type = "video"
                            elif any("Audio" in n for n in attr_names):
                                media_type = "audio"
                            elif any("Animated" in n for n in attr_names):
                                media_type = "animation"
                            elif any("Sticker" in n for n in attr_names):
                                media_type = "sticker"
                            else:
                                media_type = "document"
                        elif "Unsupported" in raw_cls or "Empty" in raw_cls:
                            media_type = None  # truly undownloadable

                # ── Text-only message ─────────────────────────────────────────
                if not media_type:
                    # Try high-level .text first, then raw.message as fallback
                    raw_text = src_msg.text
                    if raw_text is None:
                        raw_obj = getattr(src_msg, "raw", None)
                        raw_str = getattr(raw_obj, "message", None) if raw_obj else None
                        if raw_str:
                            raw_text = raw_str
                            logger.info("[DLBOT] msg %d/%d text from raw.message", i, total)

                    if not raw_text or not str(raw_text).strip():
                        logger.info("[DLBOT] msg %d/%d id=%d: no content — skipping", i, total, src_msg.id)
                        if i < total:
                            await asyncio.sleep(0.5)
                        continue

                    try:
                        await status.edit_text(f"📨 <b>Sending text {i}/{total}…</b>")
                    except Exception:
                        pass

                    # Try HTML (only works if raw_text is pyrofork Str, not plain str)
                    html_text: Optional[str] = None
                    try:
                        html_text = raw_text.html if hasattr(raw_text, "html") else None
                        if html_text and not html_text.strip():
                            html_text = None
                    except Exception:
                        html_text = None

                    plain_text = str(raw_text)
                    send_text  = html_text or plain_text
                    _parse     = ParseMode.HTML if html_text else None
                    _ok        = True

                    for chunk in [send_text[j:j + 4096] for j in range(0, len(send_text), 4096)]:
                        try:
                            await bot.send_message(
                                uid, chunk,
                                parse_mode=_parse,
                            ...
                            if _parse:
                                # HTML rejected — retry same chunk as plain
                                logger.warning("[DLBOT] HTML rejected msg %d, retrying plain: %s", i, e)
                                idx = [send_text[j:j + 4096] for j in range(0, len(send_text), 4096)].index(chunk)
                                p = plain_text[idx * 4096:(idx + 1) * 4096]
                                try:
                                    await bot.send_message(uid, p, protect_content=_protect, disable_web_page_preview=True)
                                except Exception as e2:
                                    logger.error("[DLBOT] Plain fallback failed msg %d: %s", i, e2)
                                    _ok = False
                                    break
                            else:
                                logger.error("[DLBOT] Text send failed msg %d: %s", i, e)
                                _ok = False
                                break
                ...
                # ── Media message ─────────────────────────────────────────────
                try:
                    await status.edit_text(f"📥 <b>Downloading {i}/{total}…</b>  (type: {media_type})")
                except Exception:
                    pass

                try:
                    file_path = await user.download_media(src_msg, file_name=Config.DOWNLOAD_DIR + "/")
                except FloodWait as fw:
                    await status.edit_text(f"⏳ Download rate limited. Waiting {fw.value}s…")
                    await asyncio.sleep(fw.value + 1)
                    file_path = await user.download_media(src_msg, file_name=Config.DOWNLOAD_DIR + "/")
                ...
                # Refine media_type from file extension (for raw-detected types)
                if media_type in ("photo", "document"):
                    ...
                    if ext in (".jpg", ".jpeg", ".png", ".webp", ".bmp"):
                        ...
                        media_type = "video"
                    elif ext in (".mp3", ".flac", ".m4a", ".aac", ".wav", ".opus"):
                        media_type = "audio"
                ...
                # Caption
                custom_cap = await redis_get_caption(uid)
                caption    = (custom_cap or (src_msg.caption.html if src_msg.caption else "") or "")[:1024]

                try:
                    await status.edit_text(f"⬆️ <b>Uploading {i}/{total}…</b>  📦 {humanbytes(file_size)}")
                except Exception:
                    pass

                kw = dict(chat_id=uid, caption=caption, parse_mode=ParseMode.HTML, protect_content=_protect)
                try:
                    if media_type == "video":
                        v = src_msg.video
                        await bot.send_video(
                        ...
                        vn = src_msg.voice
                        await bot.send_voice(voice=file_path, duration=vn.duration if vn else 0, **kw)
                    elif media_type == "video_note":
...
async def login_flow_interceptor(_: Client, message: Message) -> None:
    ...
    # ── Owner settings input (runs before login state check) ─────────────────
    if uid == Config.OWNER_ID and message.text:
        try:
            owner_state = await redis_get_owner_state(uid)
        except Exception:
            owner_state = None
        if owner_state:
            step = owner_state.get("step")
            if step == "awaiting_response_text":
                await _handle_response_text_input(message, uid)
                raise StopPropagation
            elif step == "awaiting_timer_custom":
                await _handle_timer_custom_input(message, uid)
                raise StopPropagation
    ...
    try:
        ...
        try:
            await status.edit_text(
                ...
                "⚠️ <b>Do NOT forward</b> the Telegram notification — "
                "forwarding the message invalidates the code.\n"
                "Send <code>cancel</code> to abort.",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton(
...
async def _login_handle_otp(message: Message, uid: int, state: dict) -> None:
    ...
    try:
        ...
        await status.edit_text(
            "⏰ <b>Code rejected by Telegram.</b>\n\n"
            "This usually happens for one of these reasons:\n"
            "• The code expired (Telegram codes last only a few minutes)\n"
            "• The code was previously used in an earlier attempt\n"
            "• The Telegram notification containing the code was forwarded — "
            "Telegram blocks codes that have been shared or forwarded\n\n"
            "⚠️ <b>Important:</b> Never forward the Telegram login notification "
            "to any chat — not even Saved Messages.\n\n"
...
# ═══════════════════════════════════════════════════════════════════════════════
#  BOT RESPONSE SETTINGS (owner only)
#  Lets the owner configure what non-owner users receive when they message.
# ═══════════════════════════════════════════════════════════════════════════════

async def _bot_response_menu_text() -> tuple[str, InlineKeyboardMarkup]:
    """Build the response settings panel text + keyboard."""
    try:
        text  = await redis_get_bot_response()
        timer = await redis_get_bot_response_timer()
    except Exception:
    ...
    panel = (
        "⚙️ <b>Bot Response Settings</b>\n\n"
        "<b>Current message:</b>\n"
        "──────────────────────\n"
        f"{preview}\n"
        "──────────────────────\n\n"
        f"<b>Auto-delete timer:</b> {_format_timer(timer)}"
    )
    kb = InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✏️ Edit Text",   callback_data="brs_edit_text"),
            InlineKeyboardButton("⏱ Set Timer",    callback_data="brs_timer_menu"),
        ],
        [InlineKeyboardButton("👁 Preview",        callback_data="brs_preview")],
        [InlineKeyboardButton("🔙 Back",           callback_data="cb_start")],
    ])
    return panel, kb


@bot.on_callback_query(filters.regex(r"^bot_response_menu$"))
async def cb_bot_response_menu(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    await query.answer()
    panel, kb = await _bot_response_menu_text()
    await edit_menu(query.message, panel, reply_markup=kb)


@bot.on_callback_query(filters.regex(r"^brs_edit_text$"))
async def cb_brs_edit_text(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    await query.answer()
    uid = query.from_user.id
    try:
        await redis_set_owner_state(uid, {"step": "awaiting_response_text"})
    except Exception:
        pass
    await edit_menu(
        query.message,
        "✏️ <b>Edit Bot Response Text</b>\n\n"
        "Send your new response message.\n\n"
        "<b>Supports full HTML formatting:</b>\n"
        "• <code>&lt;b&gt;bold&lt;/b&gt;</code>\n"
        "• <code>&lt;i&gt;italic&lt;/i&gt;</code>\n"
        "• <code>&lt;a href='url'&gt;link&lt;/a&gt;</code>\n"
        "• <code>&lt;code&gt;monospace&lt;/code&gt;</code>\n\n"
        "Links will have no preview.\n\n"
        "Send <code>/cancel</code> to abort.",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ Cancel", callback_data="bot_response_menu")
        ]]),
    )


async def _handle_response_text_input(message: Message, uid: int) -> None:
    """Handle owner's new response text input."""
    text = (message.text or "").strip()

    if text.lower() in ("/cancel", "cancel"):
        await redis_clear_owner_state(uid)
        try: await message.delete()
        except Exception: pass
        panel, kb = await _bot_response_menu_text()
        await send_menu(uid, panel, reply_markup=kb)
        return

    if not text:
        await bot.send_message(uid, "⚠️ Text cannot be empty. Try again or send /cancel.")
        return

    try:
        await redis_set_bot_response(text)
        await redis_clear_owner_state(uid)
    except Exception as e:
        await bot.send_message(uid, f"❌ <b>Failed to save:</b>\n<code>{e}</code>")
        return
    ...
    await send_menu(
        uid,
        "✅ <b>Response text updated!</b>\n\n" + panel,
        reply_markup=kb,
    )


@bot.on_callback_query(filters.regex(r"^brs_timer_menu$"))
async def cb_brs_timer_menu(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    await query.answer()
    try:
        current = await redis_get_bot_response_timer()
    except Exception:
        current = 0
    await edit_menu(
        query.message,
        f"⏱ <b>Auto-Delete Timer</b>\n\n"
        f"Current: <b>{_format_timer(current)}</b>\n\n"
        "How long after sending should the bot response auto-delete?\n"
        "Select a preset or enter a custom value:",
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("🚫 Never",    callback_data="brs_timer_0"),
                InlineKeyboardButton("30s",         callback_data="brs_timer_30"),
                InlineKeyboardButton("1 min",       callback_data="brs_timer_60"),
            ],
            [
                InlineKeyboardButton("5 min",       callback_data="brs_timer_300"),
                InlineKeyboardButton("10 min",      callback_data="brs_timer_600"),
                InlineKeyboardButton("30 min",      callback_data="brs_timer_1800"),
            ],
            [
                InlineKeyboardButton("1 hour",      callback_data="brs_timer_3600"),
                InlineKeyboardButton("✏️ Custom…",  callback_data="brs_timer_custom"),
            ],
            [InlineKeyboardButton("🔙 Back",        callback_data="bot_response_menu")],
        ]),
    )


@bot.on_callback_query(filters.regex(r"^brs_timer_(\d+)$"))
async def cb_brs_timer_preset(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    seconds = int(query.matches[0].group(1))
    try:
        await redis_set_bot_response_timer(seconds)
    except Exception as e:
        await query.answer(f"Failed: {e}", show_alert=True)
        return
    await query.answer(f"✅ Timer set to {_format_timer(seconds)}")
    panel, kb = await _bot_response_menu_text()
    await edit_menu(query.message, panel, reply_markup=kb)


@bot.on_callback_query(filters.regex(r"^brs_timer_custom$"))
async def cb_brs_timer_custom(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    await query.answer()
    uid = query.from_user.id
    try:
        await redis_set_owner_state(uid, {"step": "awaiting_timer_custom"})
    except Exception:
        pass
    await edit_menu(
        query.message,
        "✏️ <b>Custom Timer</b>\n\n"
        "Send the number of <b>seconds</b> for the auto-delete timer.\n\n"
        "<b>Examples:</b>\n"
        "• <code>45</code> — 45 seconds\n"
        "• <code>120</code> — 2 minutes\n"
        "• <code>7200</code> — 2 hours\n\n"
        "Send <code>/cancel</code> to abort.",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("❌ Cancel", callback_data="brs_timer_menu")
        ]]),
    )


async def _handle_timer_custom_input(message: Message, uid: int) -> None:
    """Handle owner's custom timer value input."""
    text = (message.text or "").strip()
    ...
    if not text.isdigit() or int(text) < 0:
        await bot.send_message(
            uid,
            "⚠️ Please send a positive whole number (seconds).\n"
            "<b>Example:</b> <code>120</code>\n\n"
    ...
    await send_menu(
        uid,
        f"✅ <b>Timer set to {_format_timer(seconds)}!</b>\n\n" + panel,
        reply_markup=kb,
    )


@bot.on_callback_query(filters.regex(r"^brs_preview$"))
async def cb_brs_preview(_: Client, query: CallbackQuery) -> None:
    if query.from_user.id != Config.OWNER_ID:
        await query.answer("Not authorised.", show_alert=True)
        return
    ...
    try:
        preview_msg = await bot.send_message(
            uid,
            f"👁 <b>Preview</b> (exactly what users will see):\n\n{text}",
            protect_content=True,
            disable_web_page_preview=True,
        )
        timer_note = f"\n\n<i>Auto-deletes after {_format_timer(timer)}</i>" if timer > 0 else ""
        if timer_note:
            try:
                ...
                pass
        if timer > 0:
            schedule_delete(preview_msg, delay=timer)
    except Exception as e:
        await bot.send_message(uid, f"❌ Preview failed:\n<code>{e}</code>")
...
async def main() -> None:
    ...
    # 3. Register bot commands with scopes:
    #    - Everyone sees nothing (empty list via default scope)
    #    - Owner sees full command list via chat-specific scope
    try:
        from pyrogram.types import (
            BotCommand,
        ...
        _owner_cmds = [
            ...
            BotCommand("pfp",            "📷 Download profile photos"),
        ...
        # Clear commands for everyone (non-owners see nothing)
        await bot.set_bot_commands([], scope=BotCommandScopeDefault())
        # Set full command list for owner only
        await bot.set_bot_commands(
            _owner_cmds,
            scope=BotCommandScopeChat(chat_id=Config.OWNER_ID),
        )
        logger.info("Bot commands registered ✓ (owner-scoped)")
    except Exception as e:
