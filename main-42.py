...
_ALL_CMDS = [
    ...
    "backup", "restore", "cancel_restore", "recover",
]
...
async def handle_story_download(message: Message, url: str) -> None:
    ...
    try:
        ...
        try:
            ...
            if any(k in err for k in ("private", "not found", "stories_not_found",
                ...
                await status.edit_text(
                    ...
                    "• The account is private and the session isn't following them\n"
        ...
        # raw_media is a raw MTProto Photo or Document — download_media() needs
        # a file_id string or high-level object.  We try multiple strategies:

        # Strategy 1: encode a FileId string and pass to download_media
        try:
            ...
            if media_type == "photo":
                ...
                fid = FileId(
                    ...
                    dc_id=raw_media.dc_id,
                    media_id=raw_media.id,
                    access_hash=raw_media.access_hash,
                    file_reference=raw_media.file_reference,
                    thumbnail_size=best.type,
                )
            else:
                fid = FileId(
        ...
        # Strategy 2: _download_file with InputFileLocation (internal Pyrogram API)
            ...
            try:
                ...
                if media_type == "photo":
                    ...
                    loc  = InputPhotoFileLocation(
                        id=raw_media.id, access_hash=raw_media.access_hash,
                        file_reference=raw_media.file_reference, thumb_size=best.type,
                    )
                    sz   = getattr(best, 'size', 0)
                    dc   = raw_media.dc_id
                else:
                    loc  = InputDocumentFileLocation(
                        id=raw_media.id, access_hash=raw_media.access_hash,
                        file_reference=raw_media.file_reference, thumb_size="",
                    )
                    sz   = getattr(raw_media, 'size', 0)
                    dc   = raw_media.dc_id

                # Try keyword-arg signature first, then positional
                for dl_args in [
                    dict(location=loc, file_size=sz, dc_id=dc,
                         file_name=tmp_path_target, progress=dl_cb),
                    dict(location=loc, file_size=sz, dc_id=dc,
                         file_name=tmp_path_target),
                    (loc, sz, dc, tmp_path_target),
                ]:
                    try:
                        if isinstance(dl_args, dict):
                            tmp_path = await user._download_file(**dl_args)
                        else:
                            tmp_path = await user._download_file(*dl_args)
        ...
        if not tmp_path or not os.path.exists(tmp_path):
            await status.edit_text("❌ <b>Download failed</b> — file not created.")
            schedule_delete(status)
            return
        ...
        try:
            if media_type == "photo":
                ...
                try:
                    ...
                    # Fallback: send as document if video send fails
...
async def handle_telegram_link(_: Client, message: Message) -> None:
    ...
    if sent:
        ...
        # process_single_message always edits status with a specific error on failure.
        # Just schedule deletion — do NOT overwrite the specific error with a generic one.
        # (status.text always reflects the original sent text, not the edited text,
        #  so the old "if 'Processing' in status.text" check was always True and always
        #  clobbered the specific error message.)
        schedule_delete(status)
    raise StopPropagation
